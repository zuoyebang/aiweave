...
```
用 design-spec 六视角做架构决策  ──→  产出技术方案(TRD)
        │
        ▼
...
| 场景 | 触发 | 输入 | AI 任务 |
|------|------|------|---------|
| **A0 — design**（技术方案设计 / 上游） | 需求待定架构 | 业务需求 + design-spec | 过六视角（`/design-solution`）产出技术方案(TRD) → 喂给 A1 / B1 |
...
| 视角 | 决策什么（含高性能极致点） |
|------|--------------------------|
| **IO** ⭐ | 三维度（次数 / 串并行 / 往返）+ 读写路径分治 + 三条铁律 + 聚合器五阶段 + 批量 Upsert / 装载 |
| **数据建模** | 数据访问层范式 / 分库分表 / shard 分组并行 / **覆盖索引 + ICP** / 组装层 map 入参 |
...
**执行器**：`/design-solution {需求}` —— 过六视角产出 TRD，再进 A1 forward / B1 sync 落地为 docs/ 与代码。
...
用 design-spec 六视角对需求做架构决策，产出 TRD（简单 CRUD 可跳过，直接进阶段 A）：

```
/design-solution {需求}  →  过 IO / 数据建模 / 并发 / 事务 / 缓存 / 稳定性 六视角  →  技术方案(TRD)
```
