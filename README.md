...
| 场景 | 触发 | 输入 | AI 任务 |
|------|------|------|---------|
| **A0 — design**（技术方案设计 / 上游） | 需求待定架构 | 业务需求 + design-spec | 过七视角（`/design-solution`）产出技术方案(TRD) → 喂给 A1 / B1 |
...
`design-spec/` 回答 docs-spec 不回答的问题——**面对需求，怎么"做出"架构决策**，而不是决策定了之后怎么写下来。它是"技术方案(TRD)生成"的参考，按 **七大决策视角（lens）** 组织，并内置一套从生产实践沉淀的**高性能基线**：
