...
放在 `docs/architecture/` 下。具体业务命名（如 `auth_flow.md` / `account_flow.md`）见末尾 §14 参考示例。
...
> 真实业务的完整流程图（如业务校验链 Verify Controller → checkBilling）见末尾 §14 参考示例。
...
> 真实业务示例（如业务校验链中的余额超扣）见末尾 §14 参考示例。
...
> 真实业务的反向对照（含余额已扣 / pickMode 等）见末尾 §14 参考示例。
