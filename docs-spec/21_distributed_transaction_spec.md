...
>
> 本规范是 AIWeave v1.2 演进新增的 P0 规范之一，对应痛点 #4 事务边界与补偿、#2 跨服务调用链。
...
- `## 5.` 最终一致性窗口（`§5.1` 窗口承诺 + `§5.2` 弱一致边界兜底登记 + `§5.3` delta flush 不变量登记 = 表示记录槽位）
- `## 6.` 失败路径全景图（强制）
- `## 7.` 与 Service 伪代码的关系（伪码标记语法）
- `## 8.` 维护流程（含 B1 反向同步规则）

> **表示记录槽位 vs 决策方法**：`§4.4` / `§5.2` / `§5.3` 是**工程填空"选了什么"的记录槽**（待填表 + `{占位符}`），不承载"何时该这么选"的决策树——决策方法论的真相源在 [`design-spec/04 §3`](../design-spec/04_transaction_design.md)，槽位仅一行反向引用回链，不复制。
...
`§5` 含三个子节：`§5.1` 一致性窗口承诺（窗口时长 + 读取策略），`§5.2` / `§5.3` 为**高频数值变更**的两个**表示记录槽位**（见 §7.1 / §7.2）。
