...
"AI 可重建工程"不仅要求行为等价，也要求**重建出的代码默认就是 IO 高效的**。一个把请求放大成 O(N) 次往返的实现，即便行为正确也不可接受。因此 IO 聚合是 AIWeave 的一等纪律。
...
涉及"对集合逐元素访问资源"或"多个跨实例 / 跨数据源读取"时，**默认走聚合器**（收集 → 按实例分组批量读 → 并行回源 → 单飞合并 → 异步写回），把分散 IO 重写为"批量 + 并行"。聚合器的共享结果指针**只读**，禁止就地改字段。

完整规则见 [`docs-spec/25_io_aggregation_spec.md`](docs-spec/25_io_aggregation_spec.md)；落地约束清单与 grep 锚见 `io_contract.md`。两条铁律由 `/io-review` 审计（L4）+ Hooks（L0，见 §14）双层兜底。
...
| Hook 族 | 事件时机 | 职责 |
| --- | --- | --- |
| **IO 铁律检查 Hook** | 编辑 / 提交 `.go` 文件后 | 机械跑 `R-IO-*` grep 锚，命中两条铁律（N+1 / 串行编排）→ 告警 / 阻断，提示走聚合器 |
